...
async def search_causes(
    ...
    if fuzzy and (case_no or petitioner or respondent or advocate or opposite_advocate):
        ...
        for cause in all_causes:
            ...
            if opposite_advocate and cause.opposite_advocate:
                s = calculate_similarity(opposite_advocate, cause.opposite_advocate)
                if s > 0.7:
                    score += s
                    matches += 1
        ...
        if query:
            query_obj = query_obj.filter(
                or_(
                    ...
                    Cause.opposite_advocate.ilike(f"%{query}%"),
        ...
        if opposite_advocate:
            query_obj = query_obj.filter(Cause.opposite_advocate.ilike(f"%{opposite_advocate}%"))
        if court_no:
...
async def download_causes_pdf(
    ...
    try:
        ...
        data = [['Court No', 'Case No', 'Petitioner', 'Respondent', 'Advocate', 'Opp. Advocate', 'Date']]
        ...
        for cause in causes:
            data.append([
                ...
                (cause.opposite_advocate or "")[:30] + "..." if len(cause.opposite_advocate or "") > 30 else (cause.opposite_advocate or ""),
                str(cause.hearing_date)
            ])
            
        table = Table(data, colWidths=[70, 90, 130, 130, 130, 130, 70])
