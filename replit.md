...
- `GET /discover-courts` - Discover available courts for a specific date (admin only)
- `POST /fetch-court-data` - Fetch and save data from selected courts (admin only)
...
## Recent Changes
- 2025-11-25: **NEW FEATURE - JSON API Migration & Opposite Advocate Support**
  - ✅ **Migrated from PDF to JSON API**: Now using https://mhc.tn.gov.in/judis/clists/clists-madras/api/result.php
  - ✅ **Added opposite_advocate column**: Captures respondent-side lawyers (mradv/exradv from JSON)
  - ✅ **Implemented sanitize_text()**: Handles list→string conversion preventing SQLite binding errors
  - ✅ **Court Discovery System**: Discover available courts and selectively fetch data
  - ✅ **Backend Functions** (scraper.py):
    - `sanitize_text()` - Ensures all text fields are plain strings, handles lists/tuples
    - `discover_available_courts()` - Scans for courts with active cause lists using JSON API
    - `fetch_full_cause_list_json()` - Fetches complete JSON data for a date
    - `process_court_cases()` - Extracts cases for specific court from JSON
  - ✅ **New API Endpoints**:
    - `GET /api/scraper/discover-courts` - Discovers available courts for a date
    - `POST /api/scraper/fetch-court-data` - Fetches and saves data from selected courts
  - ✅ **Admin Dashboard**: Court discovery with checkbox selection, bulk select/deselect
  - ✅ **Search Enhancement**: Added opposite advocate filter and display in results table
